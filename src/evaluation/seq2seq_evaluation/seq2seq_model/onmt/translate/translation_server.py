    ...
    def wrapper(server_model, *args, **kwargs):
        if sys.version_info[0] == 3:
            if not server_model.running_lock.acquire(True, 120):
                raise ServerModelError("ClassifierModel %d running lock timeout"
    ...
    def preload_model(self, opt, model_id=None, **model_kwargs):
        ...
        if model_id is not None:
            if model_id in self.models.keys():
                raise ValueError("ClassifierModel ID %d already exists" % model_id)
        else:
...
class ServerModel(object):
    ...
    Args:
        ...
        model_id (int): ClassifierModel ID
    ...
    def run(self, inputs):
        ...
        if not self.loading_lock.is_set():
            self.logger.info(
                "ClassifierModel #%d is being loaded by another thread, waiting"
                % self.model_id)
            if not self.loading_lock.wait(timeout=30):
                raise ServerModelError("ClassifierModel %d loading timeout"
                                       % self.model_id)
