from src.modelling.NER_module.models.normalizers import clean, cache
...
class MohaverekhanBasicNormalizer():
    ...
    basic_patterns_dont_keep = [(p[0], p[1]) for p in basic_patterns_dont_keep]
    basic_patterns_keep = [(p[0], p[1]) for p in basic_patterns_keep]
    basic_patterns_dont_keep = cache.compile_patterns(basic_patterns_dont_keep)

    basic_patterns_keep = cache.compile_patterns(basic_patterns_keep)

    def do_basic_patterns(self, text_content,keep_hashtag):
        if keep_hashtag:
            basic_patterns = self.basic_patterns_keep
        else:
            basic_patterns = self.basic_patterns_dont_keep
    ...
    def normalize(self, text_content, keep_hashtags):
        import re
        ...
        if keep_hashtags:
            ...
            for i in range(len(text_content)):
                if text_content[i] == '#':
                    ...
                    cnt = 0
                elif text_content[i] == '_' and status:
                    cnt += 1
            for i in range(max_cnt):
                text_content = re.compile(r'#(\w+)_').sub(rf'#\1{dummy_thing}', text_content)
        text_content = text_content.strip(' ')

        text_content = self.uniform_signs(text_content)
        # self.logger.info(f'>> uniform_signs : \n{text_content}')

        text_content = self.do_basic_patterns(text_content,keep_hashtags)
        # self.logger.info(f'>> do_basic_patterns : \n{text_content}')
        if keep_hashtags:
            text_content = re.compile(rf'{dummy_thing}').sub(r'_', text_content)
