...
import os
from src.modelling.NER_module.utils.IO import read_bijan_files, load_obj, save_obj

if os.path.isfile(path.mohaverekhan_parsed_dir + ".pkl"):
    # tag_set_token_tags = load_obj(path.mohaverekhan_parsed_fir)
    all_token_tags = load_obj(path.mohaverekhan_parsed_dir)
else:
    file = read_bijan_files(path.bijan_files_dir)
    save_obj(file, path.mohaverekhan_parsed_dir)
